...
#ifndef MSMNN_Matrix_DEFINED
#define MSMNN_Matrix_DEFINED

#include <string.h>
#include <cstdint>
#include <MSMNN/Rect.h>
...
public:
    ...
    */
    void mapPoints(Point dst[], const Point src[], int count) const {
        MSMNN_ASSERT((dst && src && count > 0) || 0 == count);
        // no partial overlap
        MSMNN_ASSERT(src == dst || &dst[count] <= &src[0] || &src[count] <= &dst[0]);
    ...
    */
    friend MSMNN_PUBLIC bool operator==(const Matrix& a, const Matrix& b);
...
private:
    ...
    */
    ...
    void setTypeMask(int mask) {
        ...
        MSMNN_ASSERT(kUnknown_Mask == mask || (mask & kAllMasks) == mask ||
    ...
    void orTypeMask(int mask) {
        MSMNN_ASSERT((mask & kORableMasks) == mask);
        fTypeMask = (uint8_t)(fTypeMask | mask);
    }

    void clearTypeMask(int mask) {
        // only allow a valid mask
        MSMNN_ASSERT((mask & kAllMasks) == mask);
