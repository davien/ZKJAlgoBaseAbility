...
//  MSMNN
//
//  Created by MSMNN on 2019/07/25.
//  Copyright © 2018, Alibaba Group Holding Limited
//
#ifndef MSMNN_Executor_hpp
#define MSMNN_Executor_hpp
#include <MSMNN/ErrorCode.hpp>
#include <MSMNN/expr/Expr.hpp>
#include <MSMNN/Tensor.hpp>
#include <MSMNN/Interpreter.hpp>
#include <vector>
#include <mutex>
#include <set>
...
public:
    ...
    void setGlobalExecutorConfig(MSMNNForwardType type, const BackendConfig& config, int numberThread);
    ...
    static std::shared_ptr<Executor> newExecutor(MSMNNForwardType type,
    ...
    class MSMNN_PUBLIC RuntimeManager {
    public:
        ~RuntimeManager();
        /**
         ...
         */
        void updateCache();
        std::vector<bool> isBackendSupport(const std::vector<MSMNNForwardType> type);
...
private:
    Executor(std::shared_ptr<Runtime> backend, MSMNNForwardType type, int numberThread);
    void _makeCache(const std::vector<EXPRP>& outputs, bool forceCPU);

    std::map<std::pair<MSMNNForwardType, int>, std::shared_ptr<Runtime>> mRuntimes;
