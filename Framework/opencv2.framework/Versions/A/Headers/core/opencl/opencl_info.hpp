...
static String getDeviceTypeString(const mscv::ocl::Device& device)
{
    if (device.type() == mscv::ocl::Device::TYPE_CPU) {
        return "CPU";
    }

    if (device.type() == mscv::ocl::Device::TYPE_GPU) {
...
static void dumpOpenCLInformation()
{
    ...
    try
    {
        ...
        mscv::ocl::getPlatfomsInfo(platforms);
        if (platforms.empty())
        {
        ...
        for (size_t i = 0; i < platforms.size(); i++)
        {
            ...
            for (int j = 0; j < platform->deviceNumber(); j++)
            {
                ...
                DUMP_CONFIG_PROPERTY( mscv::format("cv_ocl_platform_%d_device_%d", (int)i, j ),
                    mscv::format("(Platform=%s)(Type=%s)(Name=%s)(Version=%s)",
