...
/** Returns string of mscv::Mat depth value: CV_8U -> "CV_8U" or "<invalid depth>" */
CV_EXPORTS const char* depthToString(int depth);

/** Returns string of mscv::Mat depth value: CV_8UC3 -> "CV_8UC3" or "<invalid type>" */
...
/** Returns string of mscv::Mat depth value: CV_8U -> "CV_8U" or NULL */
CV_EXPORTS const char* depthToString_(int depth);

/** Returns string of mscv::Mat depth value: CV_8UC3 -> "CV_8UC3" or mscv::String() */
CV_EXPORTS const mscv::String typeToString_(int type);
...
#define CV__DEFINE_CHECK_CONTEXT(id, message, testOp, p1_str, p2_str) \
    static const mscv::detail::CheckContext CV__CHECK_LOCATION_VARNAME(id) = \
...
#define CV__CHECK(id, op, type, v1, v2, v1_str, v2_str, msg_str) do { \
    if(CV__TEST_##op((v1), (v2))) ; else { \
        CV__DEFINE_CHECK_CONTEXT(id, msg_str, mscv::detail::TEST_ ## op, v1_str, v2_str); \
        mscv::detail::check_failed_ ## type((v1), (v2), CV__CHECK_LOCATION_VARNAME(id)); \
    } \
} while (0)

#define CV__CHECK_CUSTOM_TEST(id, type, v, test_expr, v_str, test_expr_str, msg_str) do { \
    if(!!(test_expr)) ; else { \
        CV__DEFINE_CHECK_CONTEXT(id, msg_str, mscv::detail::TEST_CUSTOM, v_str, test_expr_str); \
        mscv::detail::check_failed_ ## type((v), CV__CHECK_LOCATION_VARNAME(id)); \
    } \
} while (0)
