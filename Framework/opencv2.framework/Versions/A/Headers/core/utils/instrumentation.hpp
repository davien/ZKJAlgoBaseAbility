...
#define CV_IMPL_ADD(impl)                                                   \
    if(mscv::useCollection())                                                 \
    {                                                                       \
        mscv::addImpl(impl, CV_Func);                                         \
    }
#endif
...
{
public:
    NodeData(const char* funName = 0, const char* fileName = NULL, int lineNum = 0, void* retAddress = NULL, bool alwaysExpand = false, mscv::instr::TYPE instrType = TYPE_GENERAL, mscv::instr::IMPL implType = IMPL_PLAIN);
    NodeData(NodeData &ref);
    ~NodeData();
    NodeData& operator=(const NodeData&);

    mscv::String          m_funName;
    mscv::instr::TYPE     m_instrType;
    mscv::instr::IMPL     m_implType;
    ...
    // No synchronization
    double getTotalMs()   const { return ((double)m_ticksTotal / mscv::getTickFrequency()) * 1000; }
    double getMeanMs()    const { return (((double)m_ticksTotal/m_counter) / mscv::getTickFrequency()) * 1000; }
};
