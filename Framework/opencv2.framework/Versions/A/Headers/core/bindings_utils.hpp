...
{
    return mscv::format("Int: %d", argument);
}
...
{
    return mscv::format("Float: %.2f", argument);
}

CV_WRAP static inline
String dumpDouble(double argument)
{
    return mscv::format("Double: %.2f", argument);
}

CV_WRAP static inline
String dumpCString(const char* argument)
{
    return mscv::format("String: %s", argument);
}
